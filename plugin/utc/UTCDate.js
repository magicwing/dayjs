import { padZoneStr } from './util'

export const tz = {
  DEFAULT_TIMEZONE_OFFSET: new Date().getTimezoneOffset(),
  LOCAL_TIMEZONE_OFFSET: new Date().getTimezoneOffset()
};

const datePrototype = Date.prototype

function getTimestampOffset(timezoneOffset, pastTimezoneOffset = tz.LOCAL_TIMEZONE_OFFSET) {
...
[
  ...
  UTCDate.prototype[key] = function () {
    // eslint-disable-next-line prefer-rest-params
    return datePrototype[key].apply(this.$d, arguments).replace(/GMT(.*)$/, `GMT${padZoneStr(this.$timezoneOffset)}`)
  }
})
