...
const parseDate = (date) => {
  ...
  if ((typeof date === 'string') &&
    ...
    const intYear = reg[1];
    const intMonth = reg[2] - 1;
    const intDay = reg[3] || 1;
    const intHour = reg[5] || 0;
    const intMinute = reg[6] || 0;
    const intSecond = reg[7] || 0;
    const intMilliSecond = reg[8] || 0;

    let isValid = !(isNaN(intYear) || intYear < 1900 || intYear > 9999) &&
      !(isNaN(intMonth) || intMonth < 0 || intMonth > 11) &&
      !(isNaN(intDay) || intDay < 1 || intDay > 31) &&
      !(isNaN(intHour) || intHour < 0 || intHour > 59) &&
      !(isNaN(intMinute) || intMinute < 0 || intMinute > 59) &&
      !(isNaN(intSecond) || intSecond < 0 || intSecond > 59) &&
      !(isNaN(intMilliSecond) || intMilliSecond < 0 || intMilliSecond > 999) &&
      !([3, 5, 8, 10].indexOf(intMonth) > -1 && intDay > 30);

    if (intMonth === 1) {
      if (intYear % 4 === 0 && intYear % 100 !== 0 || intYear % 400 === 0) {
        isValid = isValid && !(intDay > 29);
      } else {
        isValid = isValid && !(intDay > 28);
      }
    }
